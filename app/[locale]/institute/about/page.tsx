...
function FeatureCard({ title, description, id }: CardItem) {
  return (
    <article
      id={id}
...
export default function AboutInstitutePage() {
  ...
  return (
    <div className="min-h-screen bg-[#060a12] text-slate-200 font-sans antialiased selection:bg-cyan-500/30 selection:text-cyan-200 flex flex-col">
      ...
      <nav className="sticky top-0 z-50 border-b border-slate-800/70 bg-[#060a12] px-6 py-3.5 md:px-12">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <ul className="hidden md:flex items-center gap-9 text-[13.5px] font-medium text-slate-400 m-0 p-0 list-none">
            ...
            <li className="relative" ref={dropdownRef}>
              ...
              >
                GeoSignal Institute
                <svg
                  className={`h-3.5 w-3.5 transition-transform duration-200 ${isDropdownOpen ? 'rotate-180 text-cyan-400' : 'text-slate-500'}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                  ...
                  {DROPDOWN_LINKS.map((item) => (
                    <Link
                      ...
                      className={`block rounded-md px-3.5 py-2 text-[13px] transition-colors hover:bg-[#060a12] hover:text-white ${
                        item.href === '/institute/about' ? 'text-cyan-400 font-semibold' : 'text-slate-400'
                      }`}
                    >
                      {item.label}
                    </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className={`block h-[1.5px] w-6 bg-slate-200 transition-transform duration-200 ${isMobileMenuOpen ? 'rotate-45 translate-y-[6.5px]' : ''}`} />
              <span className={`block h-[1.5px] w-6 bg-slate-200 transition-opacity duration-200 ${isMobileMenuOpen ? 'opacity-0' : ''}`} />
              <span className={`block h-[1.5px] w-6 bg-slate-200 transition-transform duration-200 ${isMobileMenuOpen ? '-rotate-45 -translate-y-[6.5px]' : ''}`} />
            </button>
          </div>
        </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden mx-auto max-w-7xl mt-4 pb-2 border-t border-slate-800/70 pt-4">
            <ul className="flex flex-col gap-1 text-sm font-medium m-0 p-0 list-none">
              <li><Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="block rounded-md px-3 py-2.5 text-slate-400 hover:bg-[#0b1329] hover:text-white transition-colors">Home</Link></li>
              <li><Link href="/about" onClick={() => setIsMobileMenuOpen(false)} className="block rounded-md px-3 py-2.5 text-slate-400 hover:bg-[#0b1329] hover:text-white transition-colors">About</Link></li>
              <li><Link href="/services" onClick={() => setIsMobileMenuOpen(false)} className="block rounded-md px-3 py-2.5 text-slate-400 hover:bg-[#0b1329] hover:text-white transition-colors">Services</Link></li>
              <li><Link href="/contact" onClick={() => setIsMobileMenuOpen(false)} className="block rounded-md px-3 py-2.5 text-slate-400 hover:bg-[#0b1329] hover:text-white transition-colors">Contact</Link></li>
            </ul>
        ...
        <div className="relative mx-auto max-w-7xl">
          <div className="max-w-3xl">
            ...
            <p className="text-slate-400 text-base md:text-lg leading-relaxed mb-10">
              An academic and scientific platform for rigorous research, computational work, and
              responsible geoscience, run alongside the consulting side rather than as a marketing arm of it.
            </p>
      ...
      {/* MAIN CONTENT */}
      <main className="mx-auto max-w-7xl px-6 py-20 md:px-12 w-full flex-1 space-y-24">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <FeatureCard
              title="Structure"
              description="The GeoSignal Research Institute (GSRI) is the academic and scientific branch of GeoSignal Analytics LLC, set up as a dedicated space for research in geophysics, signal processing, and subsurface data analysis that doesn't have to answer to a client deadline. It sits between academic research and applied geoscience, working through problems that are still open rather than shipping a fixed methodology."
            />
            <FeatureCard
              title="Publishing and training"
              description="Work here gets published through peer review, kept open where possible, and shared through international collaboration. GSRI also runs courses and workshops for students and early-career researchers. The point of publishing is that the work can be checked by people outside the company, not taken on our word."
            />
            <FeatureCard
              title="International outlook"
              description="GSRI works with universities, research centers, and industry partners across several countries, held to the same standards as any academic group in Earth and data sciences. Being affiliated with GeoSignal Analytics gives it a direct line into applied work, while keeping the research separate from the commercial side."
            />
            <FeatureCard
              title="Scientific scope"
              description="The research leans on methodological rigor and physical interpretability over black-box results. Work here spans seismic imaging, signal processing, distributed acoustic sensing (DAS), numerical modeling, and machine learning methods for subsurface characterization, all expected to stay reproducible."
            />
          </div>
        </section>
        ...
        <section id="research" className="scroll-mt-24">
          <div className="max-w-2xl mb-14">
            ...
            <p className="text-slate-400 text-sm sm:text-base leading-relaxed">
              Advance Earth science by combining physical principles with computational methods, so
              conclusions about the subsurface rest on more than a model's confidence score.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            <FeatureCard
              title="Research"
              description="Fundamental and applied work in geophysics, signal processing, and subsurface data science, with a preference for AI methods that stay interpretable and grounded in physics rather than opaque by default."
            />
            <FeatureCard
              title="Publications and open work"
              description="Original findings shared through peer-reviewed publications and open-source code where it makes sense, at the intersection of theory, numerical modeling, and large DAS or seismic datasets."
            />
            <FeatureCard
              title="Training"
              description="Technical modules, workshops, and research programs meant to give students and industry professionals a real foundation, not just a certificate."
            />
          </div>
        </section>
        ...
        <section>
          <div className="max-w-2xl mb-14">
            ...
            <p className="text-slate-400 text-sm sm:text-base leading-relaxed">
              Nothing dramatic. More collaboration, methods that hold up over time, and research that
              actually reaches the projects it's meant to inform.
            </p>
          </div>

          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <FeatureCard
              title="Cooperation with real impact"
              description="Ongoing work with universities, public research bodies, and industry partners, aimed at sustainable resource management, hazard assessment, infrastructure monitoring, and responsible use of the subsurface."
            />
            <FeatureCard
              title="A longer time horizon"
              description="Geophysical methods that hold up matter more for energy transition, environmental questions, and hazard mitigation than a single quarter's results. We try to build for that timeline, not around it."
            />
            <FeatureCard
              title="Physics-guided methods, not black boxes"
              description="Combining physics-based modeling with AI in ways that stay traceable: hybrid methods that turn subsurface signals into something a geophysicist can actually check, not just trust."
            />
            <FeatureCard
              id="experts"
              title="People, not just publications"
              description="Rigorous research and mentorship matter more here than a headcount. Open collaboration across academia and public institutions is how the next generation of geoscientists actually gets trained."
            />
          </div>
        </section>
        ...
        {/* CTA */}
        <section>
          <div className="rounded-xl border border-slate-800/80 bg-[#0b1329] p-10 sm:p-14">
            <div className="max-w-2xl">
              <h2 className="text-2xl sm:text-4xl font-bold text-white mb-6 tracking-tight">
                Let's talk research
              </h2>

              <div className="space-y-3 text-sm text-slate-400 leading-relaxed mb-9">
                <p><strong className="text-white font-medium">Academic partnerships:</strong> joint research, faculty exchanges, institutional collaborations.</p>
                <p><strong className="text-white font-medium">Research inquiries:</strong> questions on our methods (PINNs, DAS, inversion) and recent publications.</p>
                <p><strong className="text-white font-medium">Training and workshops:</strong> professional development programs for your team.</p>
              </div>
